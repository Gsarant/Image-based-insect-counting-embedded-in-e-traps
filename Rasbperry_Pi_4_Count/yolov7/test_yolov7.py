...
test_params=[
            ...
            {'model_name':'Helicoverpa_armigera_Yolov7_color_10k_overlap',
                ...
                'load_saved_parameters':'models/Helicoverpa_yolov7.pt',
            ...
            {'model_name':'Plodia_interpunctella_Yolov7_color_10k',
                ...
                'load_saved_parameters':'models/plodia_yolov7.pt',
...
#general results
path_general_results='Yolov7_Test_10k_2/General_Results'
try:
    shutil.rmtree(path_general_results)
except:
    pass    
os.makedirs(path_general_results)
gen_res_logger=init_logger(path_general_results,'Yolov7_Test_10k_2')
        ...
        for img in full_image_path:
            ...
            for result in results.xyxy:
                for box in result:
                    ...
                    if obj.cpu().detach().float().item()==float(test_param['obj']):
                        pred_label +=1
                        image=draw_win(image,int(x0),int(y0),int(x1),int(y1),None,(255,255,255))
                   # else:
                   #     image=draw_win(image,int(x0),int(y0),int(x1),int(y1),prec,(0,255,0))
            # Update metrics
            if label > 20:
