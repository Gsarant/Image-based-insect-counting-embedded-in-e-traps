...
class Convert_onnx_tf_tflite(object):
    ...
    def create_new_converted_file(self,save_parh,orig_file_path,dest_ext='tflite',quantization=True):
        file_name=orig_file_path.split(os.sep)[-1:][0]
        file_name_without_ext=file_name.split('.')[0]
        if quantization:
            file_name_without_ext=f'{file_name_without_ext}_quant'
    ...
    def representative_data_gen(self):
        for count,image_name in enumerate(self.full_image_path[:50]):
    ...
    def convert_tf_tflite(self,quantization=True):
        ...
        path_saved_model_tflite=self.create_new_converted_file(self.export_tf_path,self.onnx_file_path,'tflite',quantization)
